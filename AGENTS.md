...
- 用户明确禁止时，不得运行任何 Gradle 命令。
- 仅修改文档、注释、非行为性配置时，默认不要主动运行 `build` 或 `test`。
- 需要验证时优先使用“最小必要验证”，例如只做静态检查、定点阅读或说明未执行测试的原因。
- 涉及 UI 行为改动时，`runIde`、`runRider`、`runClion` 比纯单元测试更有参考价值，但同样需要先看用户是否允许。
